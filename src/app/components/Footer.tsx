      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative my-8">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 bg-slate-950">
            <button
              onClick={scrollToTop}
              className="p-1.5 rounded-full bg-slate-900 border border-slate-800 hover:border-cyan-500/50 text-slate-400 hover:text-cyan-400 transition-all duration-300 group shadow-md cursor-pointer"
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-medium text-slate-500">
          <p className="order-2 md:order-1 text-center md:text-left">
            © {currentYear} <span className="text-slate-400 font-semibold">AUTOMATIZACIÓN INDUSTRIAL S.A.C.</span>{' '}
            <a href="/derechos-reservados" className="text-cyan-400 hover:underline ml-1">
              Todos los derechos reservados.
            </a>
          </p>

          <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-2 order-1 md:order-2 text-slate-400">
            <span className="font-mono bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-800 text-[11px] text-cyan-300">
              RUC: 20606132901
            </span>
            <span className="hidden sm:inline text-slate-700">•</span>
            <a href="/derechos-reservados" className="text-slate-400 hover:text-cyan-400 transition-colors">
              Derechos Reservados & Términos Legales
            </a>
          </div>
        </div>
